...
class TestUnitDrunkPolishCalculator:
    ...
    @pytest.mark.parametrize(
        'x, y, expected', [(1, '1', TypeError), (None, [], TypeError), ('1', '2', TypeError), ([1], [1], TypeError)]
    )
    ...
    @pytest.mark.parametrize(
        'x, y, expected', [(5, 3, 15), (2, 0.1, 0.2), (1, -1, -1), (10, 0, 0), ('1', 2, '11'), ([1], 2, [1, 1])]
    )
    def test_multiply_positive(self, x, y, expected):
        assert op_multiply(x, y) == expected

    @pytest.mark.parametrize('x, y, expected', [('2', '1', TypeError), (None, [], TypeError), ('1', '2', TypeError)])
    def test_multiply_negative(self, x, y, expected):
        with pytest.raises(expected):
            op_multiply(x, y)

    @pytest.mark.parametrize('x, y, expected', [(3, 5, 1.6666666666666667), (1, 2, 1), (0.1, 2, 20)])
    ...
    def test_divide_negative(self, x, y, expected):
        with pytest.raises(expected):
            op_divide(x, y)
...
class TestIntegrationDrunkPolishCalculator:
    ...
    @pytest.mark.parametrize(
        'expression, expected',
        [('', IndexError), ('2 2+', ValueError), (None, AttributeError), ('string string', ValueError)],
    )
    def test_calculate_expression_negative(self, expression, expected, monkeypatch):
        with pytest.raises(expected):
            monkeypatch.setattr('builtins.input', lambda _: expression)
            main()
